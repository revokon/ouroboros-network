...
withConnectionManager ConnectionManagerArguments {
        ...
        connectionManager =
          case connectionHandler of
            ConnectionHandler (WithInitiatorMode outboundHandler) ->
              ConnectionManager
                (WithInitiatorMode
                  (connectAndInclude stateVar mainThreadId outboundHandler))

            ConnectionHandler (WithResponderMode inboundHandler) ->
              ConnectionManager
                (WithResponderMode
                  (includeConnection stateVar mainThreadId inboundHandler Inbound))
  ...
  where
    ...
    includeConnection
        ...
        -> ThreadId m
        -- ^ main thread; It is not guaranteed that `includeConnection` will run
        -- tin the same thread  as the connection manager thread thus we need to
        -- pass it the handle to it as an argument.
            ...
            Nothing -> do
              ...
              thread <-
                mask $ \unmask -> async $ do
                  catchJust
                    -- 'AsyncCancelled's have a special treatment, this
                    -- so way we cannot shoot ourselves in a foot with
                    -- `rethrowPolicy`.
                    (\e -> case fromException e of
                        Just (_ :: AsyncCancelled) -> Nothing
                        Nothing                    -> Just e)
                    (unmask
                       (handler
                          muxPromise
                    ...
                    (\e -> case rethrowPolicy peerAddr e of
                        Nothing  -> traceWith tracer (ErrorFromHandler connectionId e)
                                 >> throwM e

                        Just e'  -> traceWith tracer (RethrownErrorFromHandler e')
                                 >> throwTo mainThreadId e'
                                 >> throwM e)
    ...
    connectAndInclude
        ...
        -> peerAddr
        -> m (STM m muxPromise)
    connectAndInclude stateVar mainThreadId handler peerAddr = do
        -- Three three stages:
        --
        ...
        case mbMuxPromise of
          ...
          Nothing -> do
            ...
            traverse_ (bind connectionSnocket socket) connectionManagerAddress
            connect connectionSnocket socket peerAddr
            includeConnection stateVar mainThreadId
                              handler Outbound
                              socket peerAddr

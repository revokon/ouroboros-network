...
-- | Classification of server side errors.
--
data ServerErrorControlCommand
      -- | error should be rethrown by the main thread
      = ShutdownNode
      -- | error should shutdown both sides of the connection; We close mux.
      | ShutdownPeer

type RethrowPolicy = SomeException -> ServerErrorControlCommand
...
data ServerArguments (muxMode :: MuxMode) socket peerAddr versionNumber versionDict bytes m a b = ServerArguments {
      ...
      serverControlChannel    :: ServerControlChannel m muxMode peerAddr
                                                      versionNumber a b,
      serverRethrowPolicy     :: RethrowPolicy
    }
  ...
  where
    ...
    monitoringThread = forever $ do
      ...
      case controlMessage of
        ...
        Server.MiniProtocolException mux ConnectionId { remoteAddress } miniProtocolNum err ->

          case serverRethrowPolicy err of
            ShutdownNode -> do
              traceWith serverTracer (ServerFatalError remoteAddress miniProtocolNum err)
              -- retethrow exception
              throwM (ExceptionInHandler remoteAddress err)

            ShutdownPeer -> do
              traceWith serverTracer (ServerPeerError remoteAddress miniProtocolNum err)
              -- stop mux, this halts both initiator and responder sides at the
              -- same time.
              Mux.stopMux mux

...
instance SingleEraBlock b => IsLedger (LedgerState (DegenFork b)) where
  type LedgerCfg (LedgerState (DegenFork b)) = LedgerCfg (LedgerState (HardForkBlock '[b]))
...
instance SingleEraBlock b => HasHardForkHistory (DegenFork b) where
  ...
  hardForkShape _ = hardForkShape (Proxy @(HardForkBlock '[b]))
  hardForkTransitions cfg (DLgr lgr) = hardForkTransitions cfg lgr
...
instance SingleEraBlock b => ValidateEnvelope (DegenFork b) where
  ...
  checkPrevHash         _ = checkPrevHash         (Proxy @b) `on` projHeaderHash
...
projCfg :: TopLevelConfig (DegenFork b) -> TopLevelConfig b
projCfg = projCfg . castTopLevelConfig
...
type instance NodeState (DegenFork b) = NodeState b

instance (SingleEraBlock b, RunNode b) => RunNode (DegenFork b) where

  nodeForgeBlock cfg upd block (Ticked slot (DLgr lgr)) txs proof =
      (DBlk . injBlock) <$>
        nodeForgeBlock
          (projCfg cfg)
          upd
  ...
  nodeTxSize (DTx tx) = nodeTxSize (projGenTx tx)
  ...
  nodeDecodeLedgerState cfg =
      (DLgr . injLedgerState (nodeStartTime cfg)) <$>
        nodeDecodeLedgerState (projCfg cfg)
  nodeDecodeConsensusState cfg =
      injConsensusState <$>
        nodeDecodeConsensusState (projCfg cfg )
